...
<script>
	...
	function bodyOnLoad()
	{
			...
			$("#form1").attr("action","/admin/codeManagement/majorManagerProcess");
			$("#form1").validate({  
					groups: {
						COMMONGRP: "MJR_NM CD_IDX"
					},
			    rules: {  
			      MJR_NM:{required:true},
			      CD_IDX:{required:true}
			    },  
			    onkeyup:false,
			    messages: {
			    		COMMONGRP:"빈항목을 채워주세요.",
			    		MJR_NM:"<?=$codeTitle?>명을 입력하세요.",
			    		CD_IDX:"구분을 선택하세요."
			    },
	...
	function modifyProcess(code)
	{
		...
		$.ajax({
    url: "/admin/codeManagement/majorManagerData?MJR_IDX=" + $("#MJR_IDX").val(),
    type: "get",
    dataType:'json',
    //data: serializedData,
    success: function(response, textStatus, jqXHR){
    
     $("#MJR_NM").val(response['MJR_NM']);
...
<input type="hidden" name="MJR_IDX" id="MJR_IDX" />
<input type="hidden" name="h_sch_gubun" id="h_sch_gubun" value="<?=$sch_gubun?>" />
<input type="hidden" name="h_sch_mjr_nm" id="h_sch_mjr_nm" value="<?=$sch_mjr_nm?>" />
<input type="hidden" name="h_per_page" id="h_per_page" value="<?=$per_page?>" />
					<div id="Sch-box">
						<div class="mgb10">
							...
							<input type="text" name="MJR_NM" id="MJR_NM" style="width:100px;"  class="required"  />
						</div>
					...
					<div id="Bd-top-sch">
						...
						<div class="sch">
							<form id="top-sch" name="top-sch" method="get" action="/admin/codeManagement/majorManagerList">
								...
								<fieldset>
									...
									<legend>게시물 검색</legend>
										...
										<input type="text" name="sch_mjr_nm" id="sch_mjr_nm" title="검색어입력" style="width:100px" value="<?=$sch_mjr_nm?>"/>
					...
					<table id="BdList-Type01" summary="목록">
						...
						?>
							<tr>
								...
								<td><?=$data->REG_DT?></td>
								<td><span class="btn03"><a href="javascript:modifyProcess('<?=$data->MJR_IDX?>');">수정</a></span></td>
								<td><span class="btn04"><a href="javascript:deleteProcess('<?=$data->MJR_IDX?>');">삭제</a></span></td>
							</tr>
						<?} 
